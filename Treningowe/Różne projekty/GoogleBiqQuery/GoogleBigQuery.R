...
boxplot(FF_M$f0_,TT_M$f0_, range=1,outline=F, main="P³eæ mêska", ylim=c(5,10),
        col=c("powderblue","powderblue"), names=c("Brak alkoholu lub papierosów","Alkohol lub papierosy"))

boxplot(FF_F$f0_,TT_F$f0_, range=1,outline=F, main="P³eæ ¿eñska", ylim=c(5,10),
...
legend("topright",legen=c("Brak alkoholu lub papierosów","Alkohol lub papierosy"), lty=c(1,1), col=c("black","red"),
       cex=0.8)

plot(density(na.omit(FF_F$f0_), bw=1.5), main = "P³eæ ¿eñska", xlim=c(0,17))
lines(density(na.omit(TT_F$f0_), bw=1.5), col="deeppink")
legend("topright",legen=c("Brak alkoholu lub papierosów","Alkohol lub papierosy"), lty=c(1,1), col=c("black","deeppink"),
       cex=0.8)
